...
def main():
    ...
    #---load all FA data
    combines_list = [x for x in os.listdir(path) if 'combine' in x if not 'st_sst' in x]
    TRAIN_DATA_FILE = [join(path, f'{x}') for x in combines_list]
    df_df = {k:v for k,v in zip(TRAIN_DATA_FILE, [pd.read_csv(y, sep = ',')['text'] for y in TRAIN_DATA_FILE])}
    df_df = pd.concat(df_df.values(), axis=0)
    ...
    combines_list = ['XN_LAMBDA_BF_PREF_2019.csv', 'XN_LAMBDA_BF_PREF_2020.csv', 'XN_LAMBDA_BF_PREF_2021.csv']
    TRAIN_DATA_FILE = [join(path, f'{x}') for x in combines_list]
    df_df = {k:v for k,v in zip(TRAIN_DATA_FILE, [pd.read_csv(y, sep = ',') for y in TRAIN_DATA_FILE])}
    df_xn_lda = pd.concat(df_df.values(), axis=0)
